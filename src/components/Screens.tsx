...
export function Settings({
  ...
  return <div className="screen-body"><section className="card section-wide settings-block"><div className="card-head"><h2 className="eyebrow">Make it yours</h2><span className="card-meta">{mode === 'empty' ? 'empty file' : 'saved on this device'}</span></div><div className="settings-row"><div><div className="settings-label">Starting balance</div><div className="settings-copy">The money you have to work with right now.</div></div><input value={balance} onChange={(e) => setBalance(e.target.value)} onBlur={() => setStartingBalance(Math.max(0, Number(balance) || 0))} inputMode="decimal" aria-label="Starting balance" /></div><div className="settings-row"><div><div className="settings-label">What to call the big number</div><div className="settings-copy">Safe to spend today</div></div><select defaultValue="safe"><option value="safe">Safe to spend today</option><option value="left">Left for today</option></select></div><div className="settings-row"><div><div className="settings-label">Appearance</div><div className="settings-copy">Light</div></div><button className="btn btn-ghost" onClick={() => document.documentElement.classList.toggle('dark')}>Toggle contrast</button></div></section><section className="card section-wide"><div className="card-head"><h2 className="eyebrow">Your data</h2><span className="card-meta">local to this device</span></div><p className="long-copy">Use the example while you learn, or start from zero whenever you are ready. Your choice stays here after a refresh.</p><div className="action-row"><button className="btn btn-ghost" onClick={onExample}>Restore the example</button><button className="btn btn-primary" onClick={onEmpty}>Start over from zero</button></div></section></div>;
}


export function Help({ onExample, onEmpty }: { onExample: () => void; onEmpty: () => void }) {
  return <div className="screen-body"><section className="card section-wide"><div className="card-head"><h2 className="eyebrow">How this planner works</h2><span className="card-meta">plain words</span></div><div className="help-list"><details open><summary>Pick a screen. Read only that.</summary><p>Today is for the one thing you need now. Planner shows the month. Bills, Money in, Goals, Debt, Envelopes and Milestones keep the rest somewhere calm.</p></details><details><summary>Use it on your phone and computer</summary><p>Your saved numbers stay on this device while it opens. Nothing here needs an account.</p></details><details><summary>Start with an example or start blank</summary><p>The example is made up. Empty file means every balance, bill, goal and category starts at zero.</p></details><details><summary>Keyboard shortcuts</summary><p>Press Ctrl or Command plus K for quick actions. Press Escape to close a dialog.</p></details></div></section><section className="card section-wide"><h2 className="eyebrow">Choose your starting point</h2><div className="action-row"><button className="btn btn-ghost" onClick={onExample}>Keep the example</button><button className="btn btn-primary" onClick={onEmpty}>Start with an empty file</button></div></section></div>;
}
